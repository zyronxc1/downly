...
import { AnalyzeResponse, FormatInfo } from '../types/api';
...
export class YtDlpService {
   ...
   */
  private normalizeResponse(info: YtDlpInfo): AnalyzeResponse {
    const rawFormats = info.formats || [];
    const processedFormats: FormatInfo[] = [];
    const seenFormats = new Set<string>(); // Track duplicates
    ...
    for (const format of rawFormats) {
      ...
      const type: 'audio' | 'video' = hasVideo ? 'video' : 'audio';
      ...
      if (seenFormats.has(formatKey)) {
        ...
        const existingIndex = processedFormats.findIndex(
          f => f.type === type && f.ext === normalizedExt && f.resolution === resolution
        );
        if (existingIndex >= 0) {
   ...
   */
  private normalizeContainerType(ext: string): string {
   ...
   */
  private normalizeResolution(format: YtDlpFormat, type: 'audio' | 'video'): string {
    // Audio formats
    if (type === 'audio') {
      return 'audio';
    }
   ...
   */
  private normalizeFileSize(filesize?: number, filesize_approx?: number): string {
   ...
   */
  private extractResolutionValue(resolution: string): number {
