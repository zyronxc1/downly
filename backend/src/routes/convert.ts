...
import { ConversionService, ConversionFormat } from '../services/conversionService';
...
export const convertHandler = async (
  ...
  try {
    ...
    let actualFormat: ConversionFormat;
    ...
    if (jobId && typeof jobId === 'string') {
      ...
      const format = target_format.toLowerCase() as ConversionFormat;

      // Add to queue
      const { jobId: newJobId, canStart } = queueService.addConvertJob(url, format);
    ...
    const streamPromise = new Promise<void>((resolve, reject) => {
      ...
      stream.on('error', (error: Error) => {
        ...
        cleanup();
        if (job) {
          queueService.failJob(job.id, error.message || 'Conversion failed');
        }
        if (!res.headersSent) {
          res.status(500).json({
            error: {
