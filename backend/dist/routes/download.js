...
const safeFilename_1 = require("../utils/safeFilename");
...
const downloadService = new downloadService_1.DownloadService();
/**
 * GET /api/download
...
const downloadHandler = async (req, res, next) => {
    try {
        ...
        if (jobId && typeof jobId === 'string') {
            ...
            // Add to queue
            const { jobId: newJobId, canStart } = queueService_1.queueService.addDownloadJob(url, format_id);
            ...
            actualDownloadId = download_id || newJobId;
        }
        // Generate or use provided download ID and create session
        const progressDownloadId = progressService_1.progressService.createSession(actualUrl, actualFormatId, actualDownloadId);
        ...
        const filename = (0, safeFilename_1.safeFilename)(downloadInfo.filename);
        ...
        const streamPromise = new Promise((resolve, reject) => {
            stream.on('end', () => {
                ...
                progressService_1.progressService.markCompleted(progressDownloadId);
                resolve();
            });
            stream.on('error', (error) => {
                // Stream error occurred
                cleanup();
                progressService_1.progressService.markError(progressDownloadId, error.message || 'Download failed');
                if (job) {
                    queueService_1.queueService.failJob(job.id, error.message || 'Download failed');
                }
                if (!res.headersSent) {
                    res.status(500).json({
                        error: {
